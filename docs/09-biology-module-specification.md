...
- **Later — illustration.** A parametric 2-D body-plan generator layers the trait vector into an image: body archetype (from organization + structure + locomotion) → appendages (count/type from motility/limb traits) → head & sensory (from sensory/diet traits) → integument & coloration (from integument trait + biome palette) → size scale. Seeded per `SpeciesId`, deterministic, same every time. **This is a consumer of Doc 04 §3.7's `SubHexTerrainGenerator`-style lazy-detail hook and is implemented with the rendering phase (Doc 14);** the contract (trait vector → deterministic 2-D creature) is fixed here.
- **Humanoid toggle** (`humanoid_sapients`, §10.3) constrains the generator for sapient species specifically (upright, bilateral, four-limbed) when set — the owner's Star-Trek dial.
...
15. **P4-15** — Rendering: biome maps, bestiary text generation, tree-of-life view; illustration contract to Doc 14 (§8.5).
