...
**Crustal roots (permanent orogenic legacy).** Continents are palimpsests: Earth's crust records every orogeny it has survived as permanently thickened, isostatically buoyant basement (the Appalachians, Urals, and Guiana shield stand on 300 My–1.7 By-old roots). Each `SurfaceFeature` carries `root_m` — a non-eroding elevation floor above the continental baseline. Collision and arc uplift bank `ROOT_BANK_FRACTION = 0.3` of every uplift meter into the root (capped at `ROOT_MAX_M = 2000` — Tibetan-scale crust saturates); crustal shortening banks its uplift into the weld; terrane accretion arrives with `ACCRETED_TERRANE_ROOT_M = 350` of arc basement. Erosion (both hydrology's §8 deltas and the dry-weathering path) may plane the surface down to `CONTINENTAL_BASE_ELEVATION_M + root_m` but never below — dead belts decay into 500–1500 m plateaus instead of featureless plain — while tectonic forcing (rifting, subsidence, shortening) can still rework or destroy the feature outright. Roots ride plate surfaces through drift and rifting, so a fragment carries its mountain history to its new continent. At formation, every continental plate is seeded with area-scaled **fossil basement belts** (deterministic per world seed, stream `fossil-basement-belts`): wandering bands of root recording the orogenies that assembled it before year 0, so no continent is born a blank pancake. Gate: `gate_continental_relief_distribution` — mountain enrichment ≤ 2.5× land share for continents holding ≥ 10% of world land; flatness is epoch-aware (deep time: at most one Australia-class flat continent, nothing larger; 1 B allows a couple of modest flats while freshly continentalized crust — §5.11 — has yet to orogenize).
...
### 5.11 Crust Supply (Continentalization)

The §5.2 shortening and §5.10 subduction erosion consume continental area every tick, and no passive mechanism replaces it: measured over deep time, coverage collapsed 37% → 9% of the sphere by 4.4 B years, forcing the Doc 06-CAL hypsometry transfer to promote featureless abyssal floor to land ("false continents" at 100–200 m with no crust, no roots, no relief). Earth's crust budget instead holds roughly steady over 4 By: arc magmatism and the continentalization of long-emergent, thickened crust replace what collisions eat.

`maintain_crust_supply` (accretion.rs) is that source term — a supply controller in the Doc 06-CAL solve-to-target spirit. Each Geological tick, if `continental_crust` coverage sits below `land_fraction + CRUST_SHELF_ALLOWANCE` (0.08 — Earth's crust covers ~40% of the surface against ~29% land; the difference is shelf and slope), the highest-standing oceanic crust converts, at most `CONTINENTALIZATION_MAX_PER_TICK` (0.0005 of cells — ~8 cells/My at subdivision 6, comfortably above the ~3 cells/My measured consumption, low enough that margin growth stays under the §7.1 coastline-stability gate) and never below `CONTINENTALIZATION_MIN_ELEVATION_M` (−1000 m — abyssal floor stays oceanic no matter the deficit). Converted crust receives young basement graded by its standing height (`continentalized_root_m`: a taller arc is thicker crust, so conversion fossilizes the edifice's shape into permanent §5.2 root), bedrock flips ophiolite-style to Igneous, and isostasy then lifts it toward the continental freeboard: continents grow at their active margins, exactly where Earth grows them. Deterministic — candidates in descending-elevation, ascending-`HexId` order, no RNG.

Measured with the controller: coverage holds at 34–41% across 4.4 B years (seeds 42/99), land-on-continental-crust recovers from 18–28% to 73–93%, and the §5.2 relief gate's flatness assertions hold without any crust-label exclusion.

### 5.12 Epeirogenic Basins and Swells (Dynamic Topography)

Measured across seeds, continental interiors developed **vast inland plains** — single connected regions of sub-200 m local relief covering up to 20% of world land (Earth's largest, the West Siberian Plain, is ~2%). Root cause, in three layers: interiors are source-free after formation (boundary uplift reaches only 4 rings inland); every interior process — freeboard-floored erosion from above, continental heal and epeirogenic rebound from below — then pinches the structure field onto the single scalar `sea + CONTINENTAL_FREEBOARD_M`; and the Doc 06-CAL rank transfer maps the resulting continent-sized rank-tied cohorts into its gentle land mid-band as uniform plains.

Earth's interiors escape this because of **mantle dynamic topography**: cratons ride ±100–300 m basins and swells at 1000+ km wavelengths (the Michigan and Williston basins, the Ozark and Cincinnati arches). `epeirogeny.rs` supplies the equivalent: a pure deterministic function of `(world seed, plate id, birth-frame position)` — signed spherical-Gaussian bumps (`SWELL_BUMPS_PER_PLATE` = 8, σ ≈ 12°, clamped ±`SWELL_CLAMP_M` = 300 m) — folded into the **attractor target** of dry erosion, epeirogenic rebound, GIA, and continental heal (`freeboard + swell`). Because it shifts the target rather than the elevation, every convergence process *preserves* the undulation instead of erasing it; because it is evaluated in the birth frame, it advects rigidly with the plate at zero storage cost. Crustal-root floors stay deliberately swell-free — a wobbling floor would flicker belt cells across the mountain band and break the §7.1 persistence guarantee.

Measured with the swell (plus the Doc 06-CAL mid-band and taper changes): the largest connected plain drops to ≤ 2% of land on the gate seeds (median across 12 seed-epochs: ~1 Mkm², from ~8 Mkm²), zero flat major continents at either epoch, and the §7.1 stability gate improves to its best values (mountain Jaccard 0.828, height drift 409 m per 10 My). Gate: `gate_no_vast_inland_plains` — no connected sub-200 m-relief region may exceed 8% of world land at 1 B or 4.4 B years.

### 5.13 Mountain Belt Distribution (Bands, Not Blobs)

Measured before this section's changes, one central collision belt held up to two-thirds of all mountain area at 700–1300 km mean width — a blob — while the rest of the planet carried scattered peaks. Earth instead grows **many long, narrow belts** (Andes, Rockies, Appalachians, Atlas, Alps, Urals…), most of them on subduction margins with no continental collision at all. Four coordinated changes:

- **Andean margins:** `OC_COASTAL_UPLIFT_FACTOR` 0.10 → 0.45 (elevation.rs). Subduction margins now build real cordilleras — the Andes are Earth's longest range and pure subduction product. Margins are long and linear, so these belts are bands by construction.
- **Ridgeline collisions:** `INLAND_FALLOFF` narrowed to an effective 2-ring swath. The old 4-ring spread built ~9-hex-wide collision zones whose rank mass hogged the hypsometric tail; the rank transfer is zero-sum, so narrowing the swath spills the orogenic tail outward to the margin cordilleras — redistribution for free.
- **Orogen band trimmed:** `OROGEN_BAND_FRAC` 0.15 → 0.07 of cells (Doc 06-CAL §7.1). The old band protected the top 52% of land at the slow decay, entrenching the incumbent blob and ratcheting mid-band ramps.
- **Residual clamp:** the Doc 06-CAL §5.4 re-injection is capped at ±400 m. Uncapped, fresh boundary uplift's high-frequency spikes inflated the ≥1500 m class to ~2× the curve's allocation and made band membership churn with per-tick uplift jitter.

Plus **dissection texture** (epeirogeny.rs `terrain_texture_m`): hex-scale (~400 km wavelength, physical and resolution-independent) relief at up to ±140 m, modulated by a province-scale ruggedness mask so some provinces read as smooth steppe/Great-Plains and the rest as dissected upland — folded into the same attractor-target channel as the §5.12 swell.

Three follow-up changes completed the picture (measured across 6 seeds × 4 epochs, verified visually on rendered maps):

- **Root decay** (`ROOT_DECAY_RATE_PER_YEAR` = 1.4e-9, e-folding ~700 My, erosion.rs): dead-belt roots relax and delaminate as on Earth (Appalachians at 300 My still stand; Caledonides at 400 My are planed; Precambrian shields are flat). Without it, roots accumulated over every collision epoch forever and the union of 3 By of sutures grew into one conglomerate mountain region spanning up to a quarter of world land, growing monotonically through mid-history.
- **Orogen-EMA decay trimmed** 150 → 70 My (Doc 06-CAL §7.1): long-term belt persistence is the roots' job; the EMA only bridges rank jitter. At 150 My it kept every hex a migrating collision front had ever swept ranked high.
- **Summit tail sharpened + Andean-grade arcs**: a curve control point at `0.97·land → modal+2700` confines ≥3000 m ground to the top ~3% of land (Earth ~1.5%; previously 8% — summit *slabs*), and `OC_COASTAL_UPLIFT_FACTOR` 0.45 → 0.80 lets subduction cordilleras reach collision-grade rank (the Andes rival the Himalaya). Margins now carry their own ranges on every seed.

Measured across the grid: 5–15 belts at every epoch, mean belt widths 3–6 hexes, deep-time largest-belt share 32–48%; mid-history supercontinents may run one connected assembly-era chain (a Central-Pangean / American-Cordillera analog — verified band-shaped on renders, not a slab). Mountain persistence: Jaccard ≥ 0.65 with drift ≤ 900 m per 10 My (margin-dominated belts churn at band edges while range cores stay put); land stability at its best (0.92). Gates: `gate_mountain_belt_distribution` — four epochs printed, deep-time asserts ≥ 4 belts, ≤ 75% share, ≤ 9 hexes width; the `render_probe` utility test writes equirect elevation maps to `$RENDER_DIR` for visual regression.
